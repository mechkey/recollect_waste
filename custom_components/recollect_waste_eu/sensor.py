...
async def async_setup_entry(
    ...
    # One combined sensor: "<bin type(s)> in <N> days" as a single state.
    async_add_entities([ReCollectWasteNextPickupSummarySensor(coordinator, entry)])
...
class ReCollectWasteNextPickupSummarySensor(ReCollectWasteEntity, SensorEntity):
    """Define a single sensor combining bin type(s) and days-to-collection."""
    ...
    def __init__(
        ...
        self._attr_unique_id = f"{self._identifier}_next_pickup_summary"
        self._attr_name = "Next Pickup Summary"
...
class ReCollectWastePickupTypeSensor(ReCollectWasteEntity, SensorEntity):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        super()._handle_coordinator_update()        translation_key=SENSOR_TYPE_NEXT_PICKUP,
    ),
)
